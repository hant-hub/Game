...
void WuDestroy(GameState* state) {
...
void WuUpdate(GameState* state, PresentInfo* p) {
    u32 frameCounter = 0;
    double dt = 0;
    double accum = 0;
    ...
    while (!glfwWindowShouldClose(sr_context.w)) {
        ...
        bool carving = KeyButtonEx(state, &c, GLFW_KEY_1, 2);
        bool typing = KeyButtonEx(state, &c, GLFW_KEY_2, 2);


        if (carving) {
            ma_PlaySound(&state->audio, CARVING_1);
        }

        if (typing) {
            ma_PlaySound(&state->audio, TYPING);
        }
